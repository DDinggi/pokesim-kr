...
const SET_THEMES: Record<string, { gradient: string; accent: string; subtitle: string }> = {
  'm4-ninja-spinner': {
...
function SetCard({
  ...
  set: SetMeta;
  theme: { gradient: string; accent: string; subtitle: string };
  onSelect: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 bg-gray-950/70 backdrop-blur">
        ...
        {theme.subtitle && (
          <p className="text-[11px] text-white/60 mt-0.5 line-clamp-1">{theme.subtitle}</p>
        )}
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      <header className="px-6 py-5 border-b border-gray-800/80">
        ...
        <p className="text-xs text-gray-500 mt-1">한국 포켓몬 TCG 박스깡 시뮬레이터 · 어떤 박스를 깰까요?</p>
      </header>
      ...
      <footer className="px-6 py-4 text-[10px] text-gray-600 text-center border-t border-gray-900">
        ⓘ 봉입률은 커뮤니티 추정치 · 포켓몬코리아는 확정 봉입을 안내하지 않습니다
      </footer>
    </div>
  );
}
