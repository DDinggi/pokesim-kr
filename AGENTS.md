...
사용자 브라우저
  ↓
Cloudflare Pages (Next.js 정적) ← public/sets/*.json, public/distrib/*
  ↓
...
사용자 브라우저
  ↓
Cloudflare Pages (정적) → Fly.io FastAPI
                              ↓
                          Upstash Redis
                          - 통계 카운터
                          - 공유 (TTL 7일)
...
1. 브라우저가 Cloudflare Pages에서 Next.js 빌드 결과 받음
...
**Tier P2: 커뮤니티 박스깡 사례**
- 디시 포카게 마이너 갤러리, 카드몬스터, 트위터/X
- 한계: 표본 작음, 운빨, 세트별 다름
- 활용: 신상 출시 후 1~2주 데이터 누적되면 갱신
...
- **D-002** 프론트는 Cloudflare Pages — Bandwidth 무제한 무료
...
- **D-138** 롤백 시나리오 박제 (git revert + Pages rollback + DB backward-compatible)
...
- [ ] 시작 세트 결정 (인기 최신 1개)
- [ ] 카드 메타: 번호, 이름(한국어), 레어도, 이미지 URL
- [ ] 영문판 매칭 ID
- [ ] 팩 슬롯 확률표 (커뮤니티 데이터 + 출처 메타)
- [ ] **박스 정보**:
  - box_size (보통 30), pack_size (보통 5)
  - box_price_krw, pack_price_krw
  - box_guarantees (추정치, 출처 메타 필수)
- [ ] **저장**: `frontend/public/sets/{code}.json` (정적)
- [ ] 카드 이미지 R2 업로드

### ★ MVP 첫 배포 (정적 사이트)

**무조건 띄운다. 백엔드 없음. Cloudflare Pages만.**
...
- **D-138** 배포 실패 시 복구 절차를 미리 박제:
  1. `git revert <문제 커밋>` → push → Cloudflare Pages 재배포 (자동)
  2. 또는 Cloudflare Pages 콘솔에서 직전 정상 deployment "Rollback" 버튼
