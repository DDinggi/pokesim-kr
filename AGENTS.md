...
- **D-144** 몬스터/트레이너 SR 슬롯 분리 — `card_type` 필드 기준. MEGA 확장팩은 트레이너/아이템 SR 확정 슬롯과 SR/SAR/MUR 슬롯을 분리한다. 일반 SV 확장팩의 UR은 트레이너/에너지 UR도 존재하므로 전체 UR 풀을 사용한다. card_type 미분류 시 전체 풀 폴백.
- **D-145** MEGA 확장팩 SR/SAR/MUR 슬롯 가중치 — pokemon-infomation.com + Samurai Sword Tokyo (2026-05) 실데이터 기반. 현재 MEGA 확장팩 공통 모델은 메인 슬롯 SR 70% / SAR 28% / MUR 2% + 추가 SR 슬롯 약 10%.
- **D-146** 추가 SR/SR+ 슬롯 모델 — SV 일반/MEGA 확장팩/블랙볼트·화이트플레어 모두 2장 이상 박스 가능성을 반영한다. SV 일반은 추가 SR 약 10%(트리플렛비트 5%), MEGA 확장은 추가 SR 약 10%, SV11 계열은 추가 SR 약 10%로 둔다. 한국판 공식 봉입률은 비공개이므로 실데이터 확보 시 갱신.
