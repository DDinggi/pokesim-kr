...
- **D-127** 일반 카드와 SR10팩/시크릿 갓팩은 같은 카드 중복 가능성을 허용한다. 다만 AR/CHR/TR/RRR처럼 같은 박스에서 여러 장 반복 보장되는 보조 히트 슬롯은 비복원 추출처럼 같은 `card_num` 중복을 우선 피한다.
- **D-128** 1팩/자판기 모드는 박스 보장 룰 + 갓팩 메커니즘은 무시하되, 세트별 박스 모델의 기대값을 1팩 히트 가중치로 환산해 사용한다.
...
5. 30팩 순회:
   - 각 슬롯에 대해:
     ...
     b. 갓팩이면 부스트된 가중치로 추첨
     c. 일반 팩이면 기본 가중치로 추첨
   - 추첨 후 해당 레어도 풀에서 카드 1장 선택. AR/CHR/TR/RRR 반복 보장 슬롯은 비복원 추출처럼 같은 `card_num`을 우선 제외한다. SR10팩/시크릿 갓팩은 중복 금지 근거가 확인되기 전까지 중복 가능성을 허용한다.
...
- C/U/R/RR 같은 일반 슬롯, 서로 독립적인 시크릿 슬롯, SR10팩/시크릿 갓팩은 같은 카드 중복 가능
