...
- ✅ 로그인 열람형 힛카드 도감 — 카드 획득은 로그인 여부와 무관하게 브라우저 `localStorage`에 저장하고, 도감 화면 열람만 Google 로그인을 요구한다. 세션 기록과 분리하며 서버/계정 컬렉션으로 확장하지 않는다.
...
**도입 중:** 구글 OAuth 클라이언트와 힛카드 도감 로그인 게이트. 운영 공개 전 Google Provider 설정,
개인정보처리방침·이용약관·회원 탈퇴를 마쳐야 한다.
...
- 시세 제보 (v2)
- 신고 (리뷰/시세)
- 힛카드 도감 열람 (획득 기록은 브라우저 `localStorage` 전용)
...
- **D-060** 로그인 부분 도입 (변경됨)
- **D-061** 서버/계정 기반 컬렉션 영구 저장 X — 힛카드 획득 기록은 브라우저 `localStorage` 전용이며, Google 로그인은 도감 화면 열람 권한에만 사용
- **D-062** 광고 X
- **D-063** 결제 X
- **D-064** 모바일 앱 X
- **D-065** 사용자 간 상호작용 X
...
- 로그인 열람형 힛카드 도감 (`frontend/lib/hitDex.ts`, `frontend/components/HitDexScreen.tsx`) — SAR/MUR/BWR/특별 힛카드를 `localStorage`에만 저장하고, Google 로그인 후 전용 도감 화면에서 MEGA/SV/소드&실드/썬&문 분류별 등록·미등록 상태 표시
...
- Google OAuth — 클라이언트·도감 로그인 게이트 구현, 운영 Provider·개인정보 페이지 설정 전
