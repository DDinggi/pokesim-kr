...
- **D-144** 몬스터/트레이너 SR 슬롯 분리 — `card_type` 필드 기준. 몬스터 슬롯: 포켓몬 SR/SAR/UR만. 트레이너 슬롯: 트레이너/에너지 SR/SAR만. card_type 미분류 시 전체 풀 폴백.
- **D-145** 세트별 몬스터 슬롯 SAR 가중치 — pokemon-infomation.com + Samurai Sword Tokyo (2026-05) 실데이터 기반. 세트별로 다름 (닌자스피너 33%, 니힐제로 16%, 인페르노X 30% 등).
- **D-146** 트레이너 슬롯 SAR 5% 고정 — 한국판 트레이너 SAR 봉입률 실데이터 없음. SAR×2 확률이 "극히 드물게 가능" (~0.8~1.7%/박스) 수준이 되도록 역산. 실데이터 확보 시 갱신.
...
- 2026-05-xx — **봉입률 로직 고도화.** 몬스터/트레이너 슬롯 card_type 분리 (D-144). 세트별 SAR 가중치 실데이터 적용 (D-145). 트레이너 SAR 5% 고정 (D-146).
