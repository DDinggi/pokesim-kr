...
개인정보처리방침·이용약관, 기록 다운로드를 구현했다. 운영 공개 전 Google Cloud의
...
- 개인정보처리방침 페이지
- 이용약관 페이지와 OAuth 시작 전 링크 노출 (Google production OAuth 정책)
- 회원 탈퇴 (즉시, grace 없음)
...
- **D-076** 개인정보는 최소 수집 — 최초 닉네임은 Google email의 `@` 앞부분을 사용하고, 사용자가 바꾼 닉네임은 Supabase Auth `display_name` 메타데이터에만 저장한다. 전체 email은 본인 마이페이지 밖에 노출하지 않는다. 개인정보처리방침·이용약관을 로그인 전 노출하고, 마이페이지에서 현재 계정의 누적 운·도감 기록을 내려받을 수 있게 한다.
...
- 2026-07-14 — **OAuth 운영 공개 준비(D-076).** 로그아웃 상태에서도 열리는 개인정보처리방침·이용약관을 추가하고 메인 하단과 Google 로그인 직전에 링크했다. 마이페이지에서는 현재 계정의 누적 운·힛카드 도감을 JSON으로 내려받을 수 있으며, Google Cloud·Supabase·Cloudflare 운영 설정과 RLS/RPC 검증 절차를 문서화했다.
