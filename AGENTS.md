...
- **D-127** 일반 카드 중복은 허용한다. 다만 SR10팩, AR/CHR/TR/RRR 다중 보장처럼 같은 묶음에서 여러 장 뽑는 슬롯은 비복원 추출처럼 같은 `card_num` 중복을 우선 피한다.
...
5. 30팩 순회:
   - 각 슬롯에 대해:
     ...
     b. 갓팩이면 부스트된 가중치로 추첨
     c. 일반 팩이면 기본 가중치로 추첨
   - 추첨 후 해당 레어도 풀에서 카드 1장 선택. SR10팩, AR/CHR/TR/RRR 다중 보장처럼 같은 묶음 슬롯은 비복원 추출처럼 같은 `card_num`을 우선 제외한다.
...
**중복 처리**:
- SR10팩, AR/CHR/TR/RRR 다중 보장처럼 같은 묶음 슬롯은 한 박스 안에서 같은 `card_num` 중복 방지
- C/U/R/RR 같은 일반 슬롯과 서로 독립적인 시크릿 슬롯은 같은 카드 중복 가능
