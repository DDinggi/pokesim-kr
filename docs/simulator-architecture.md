...
- https://pokemon-infomation.com/pull-rates-scarletex/
- https://pokemon-infomation.com/pull-rates-violetex/
- https://pokemon-infomation.com/pull-rates-snowhazard/
- https://pokemon-infomation.com/pull-rates-clayburst/
- https://pokemon-infomation.com/pull-rates-paradimtriger/
...
운 확인 화면은 시뮬 결과를 다시 샘플링하지 않는다. 개봉 이벤트에 저장된
`rarityCounts`를 `frontend/lib/luck.ts`의 기대값/분포 모델과 비교한다.

새 세트를 추가할 때는 시뮬 모델과 운 모델을 같은 PR에서 같이 맞춘다.

| 대상 | 위치 | 의미 |
| --- | --- | --- |
| 박스 시뮬 | `expansion.ts` / `hi-class.ts` | 실제 박스깡 결과 생성 |
| 1팩/자판기 시뮬 | `simulatePack()` + `expansionPackHitPool()` / `simulateSingleHiClassPack()` | 보장 슬롯을 기대값으로 환산한 낱팩 결과 생성 |
| 운 기대값 | `luck.ts#getLuckRatesForSet()` | SAR/top 기대 장수 |
| 고정 슬롯 차감 | `luck.ts#subtractBaselineCounts()` | 박스에서 평균 운으로 봐야 할 고정 슬롯 제거 |
| 운 점수 분포 | `luck.ts#getBoxScoreDistribution()` / `getPackScoreDistribution()` | 박스/팩 단위 운 점수 기준 |
| 표시 기대값 | `luck.ts#getExpectedScoredRarityCounts()` | 운 화면에 쓰는 힛카드 기대치 |

규칙:

- 박스 고정 슬롯은 운으로 보지 않는다. 예: MEGA 드림 ex의 MA 1장,
  MEGA 확장팩의 비서포트 트레이너즈 SR 1장은 베이스라인이다.
- 낮은 봉입률의 변동 슬롯은 운으로 본다. 예: SAR, BWR/MUR, 추가 SR.
- 1팩/자판기는 박스 고정 슬롯도 `1 / box_size` 확률로 환산한다.
  같은 양을 깠을 때 박스와 낱팩 운이 완전히 같을 수는 없지만, 기대값은
  같은 박스 모델에서 나와야 한다.
- SR/MA/SSR 같은 보조 힛은 운 지표에 반영하되 상한을 작게 둔다.
  SR만 많이 나와도 최상위 등급이 되지 않게 한다.
- BWR/MUR/UR/SAR/HR 같은 primary hit은 낮은 기대값일수록 운 점수를 크게 올린다.
...
```txt
MUR -> BWR -> SAR -> UR -> HR -> MA -> SR -> ACE -> AR -> RR -> R -> U -> C
```
