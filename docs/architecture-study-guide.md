...
3. 힛카드 기록은 카드 키와 획득 횟수만 별도 저장한다.
4. 운 기록 초기화와 기록 초기화는 서로 다른 사용자 의도이므로 분리한다.
...
카드 원본을 그대로 그리면 목록과 기록에서 큰 파일을 수백 장 요청하게 된다.
...
4. [힛카드 기록](../frontend/lib/hitDex.ts)
...
- 1만 DAU에서는 DB 용량만이 아니라 INSERT TPS, trigger 시간, R2 요청, 번들·기록 렌더링,
