...
- **2026-09-26 한국판 지연 등록분 전환** — [93개 이미지 검토](korean-image-review-20260926.md).
  ...
  시세 재매칭을 기록한다. 과거 이미지 주소도 최신 자산으로 연결한 뒤 `validate:korean-images -- --cdn`으로
  출처/CDN 해시·WebP 픽셀·기록 호환·등장 도달성을 검사한다. 업로드/변환의 `--cards ID1,ID2`로
  교체 대상만 처리할 수 있고, `audit:identity --cache-dir <새 경로> --cards ID1,ID2`로 새 증거를 조회한다.
