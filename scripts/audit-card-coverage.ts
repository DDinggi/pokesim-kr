...
function parseFullaheadItems(html: string, shopCode: string): FullaheadItem[] {
  ...
  const codeRegex = new RegExp(`PK-${escapeRegExp(shopCode.toUpperCase())}-([0-9]{1,3})(?:-([0-9]{1,3}))?`, "i");
...
function extractRarity(title: string): string | null {
  ...
  const match = normalized.match(/\b(BWR|MUR|GRA|SAR|CSR|CHR|SSR|AR|SR|HR|UR|ACE|K|S8AP|25TH|MA|A|S|RRR|RR|R|U|C)\b/);
  return match?.[1] ?? null;
}
